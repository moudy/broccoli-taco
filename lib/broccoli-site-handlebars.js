    ...
    return RSVP.all(targetFiles.map(function (targetFile) {
      ...
      function write (context) {
        ...
        if (util.isArray(context.page)) {
          context.page.forEach(function (ctx) {
            var json = clone(context);
            json.page = ctx;
            var finalFilepath = path.join(dirname.substring(0, dirname.lastIndexOf('/')), ctx.filename, filename);
            self.writeTemplate(finalFilepath, template, json);
          });
        } else {
